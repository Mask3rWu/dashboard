...
4. 机型选择：
   ...
   * 如果匹配到已有机型，可直接使用推荐机型。
     ...
     ![image-20260715162028370](D:\class\filesave\MarkdownPicture\image-20260715162028370.png)

   * 如果发现新格式，会自动新建机型，需要输入机型名称，选择要保留的数据类型，然后点击"**创建机型并继续**"。

     > 注意，机型新建将根据当前目录中的数据文件构建，若存在当前批次数据文件均无表头的情况，则该机型的列定义部分也将缺少表头。

     ![image-20260715161953903](D:\class\filesave\MarkdownPicture\image-20260715161953903.png)
5. 对每个待导入架次确认飞机、飞行日期和飞行记录单。若架次包含FlightRecord.xml文件，会自动预填飞行记录单

   ![image-20260715162133599](D:\class\filesave\MarkdownPicture\image-20260715162133599.png)

6. 点击对应场次的“**导入**”按钮，导入完整。

7. 导入完成后，可在“已导入飞行”列表中查看结果。

   ![image-20260715163754088](D:\class\filesave\MarkdownPicture\image-20260715163754088.png)
