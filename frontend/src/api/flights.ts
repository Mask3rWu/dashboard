...
export interface Flight extends FlightRecordFields { id: number; client_uid?: string; server_id?: number | null; local_id?: number | null; downloaded?: boolean; source_node_id?: string | null; sync_origin?: 'local' | 'server' | 'package' | string; sync_state?: 'local_only' | 'pending_upload' | 'syncing' | 'synced' | 'dirty' | 'upload_failed' | 'conflict' | 'server_cache' | 'server_deleted' | 'server_remote' | string; server_version?: number | null; last_sync_at?: string | null; sync_error_json?: string | null; name: string; aircraft_id: number; aircraft_name: string; model_id: number; model_name: string; source_path: string; session_key: string; flight_date: string; start_time: string; end_time: string; duration_sec: number; import_time: string; raw_file_count?: number; raw_import_warnings?: string; raw_warnings?: { file?: string; path?: string; error: string }[]; drone_id?: string; drone_model?: string; }
